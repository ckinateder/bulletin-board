...
class Server:
    ...
    def handle_reconnect_request(self, message_receive):
        ...
        if not user:
            message_body_send = {
                ...
                "message": "Your connection is still open, but your id doesn't match any existing users.",
            }
            message_send = MessageSend(
                message_receive.username,
                ServerCommand.Reconnect,
                False,
                False,
                message_receive.id,
                message_body_send,
        ...
        match (
            ...
            case (
                False,
                True,
            ):  # user's username changed, and the new username already exists
                message_body_send = {
                    "error_code": ServerErrorCode.NameTaken,
                    "message": ":".join(self.lobby.users.get_all_usernames()),
                }
    ...
    def handle_invalid_command_request(self, message_receive):
        message_body_send = {
            ...
            "message": f"{message_receive.command} is an invalid command.",
        }
        message_send = MessageSend(
            message_receive.username,
